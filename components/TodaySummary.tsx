...
const SHIFT_META: Record<ShiftType, { emoji: string; time: string; color: string; bg: string; border: string }> = {
  Day: {
    emoji: '☀️',
    time: '9AM – 5PM',
...
export default function TodaySummary({ todayShifts, shifts, today, onAdd, onEdit }: Props) {
  ...
  const upcomingShift = !activeShift
    ? todayShifts.find(s => s.shift === upcomingSlot && s.status === 'Not Started')
    : null
  ...
  return (
    <div className="space-y-4">
      ...
      {activeShift ? (
        <div className="rounded-2xl bg-ink text-white px-5 py-5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[11px] font-mono uppercase tracking-widest text-white/50">On Duty Now</p>
          </div>
          <p className="font-display text-5xl leading-none text-white">{activeShift.assignedTo}</p>
          ...
          {activeShift.handoverNotes && (
            <div className="mt-4 flex items-start gap-2 bg-white/10 rounded-xl px-3 py-2.5">
              ...
              <p className="text-xs text-white/70 leading-relaxed">{activeShift.handoverNotes}</p>
            </div>
          )}
        ...
        <div className="rounded-2xl border-2 border-dashed border-gray-200 px-5 py-6 text-center">
          ...
          {upcomingShift ? (
            <p className="text-sm text-muted mt-1.5">
              Next up: <span className="font-semibold text-ink">{upcomingShift.assignedTo}</span>
              &nbsp;· {SHIFT_META[upcomingShift.shift].emoji} {upcomingShift.shift}
            </p>
          ) : (
            <p className="text-sm text-muted mt-1.5">No shift assigned — tap below to add one</p>
          )}
        </div>
      )}
      ...
      <div className="flex items-center gap-2 px-1 pt-1">
        <p className="text-xs font-mono text-muted whitespace-nowrap">{format(today, 'EEEE, MMMM d')}</p>
        <div className="flex-1 h-px bg-border" />
        <p className="text-[10px] font-mono text-muted uppercase tracking-wider">All shifts</p>
      </div>
      ...
      {shifts.map(slot => {
        ...
        if (!shift) {
          return (
            ...
            >
              <div className={clsx('w-12 h-12 rounded-xl flex items-center justify-center text-xl', meta.bg)}>
                {meta.emoji}
              </div>
        ...
        return (
          ...
          >
            <div className="flex items-start gap-4">
              <div className={clsx('w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0', meta.bg)}>
                {meta.emoji}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-bold text-ink">{shift.assignedTo}</p>
                  <StatusBadge status={shift.status} />
                </div>
                <p className={clsx('text-xs font-medium mt-0.5', meta.color)}>{slot} · {meta.time}</p>
                <div className="flex items-center gap-3 mt-3 flex-wrap">
                  ...
                  {shift.issues?.length > 0 && (
                    <div className="flex items-center gap-1 text-xs text-amber-600">
                      <AlertCircle size={11} />
                      <span>{shift.issues.length} issue{shift.issues.length > 1 ? 's' : ''}</span>
                    </div>
                  )}
                </div>
                {shift.handoverNotes && (
                  <div className="mt-3 flex items-start gap-1.5 bg-black/[0.03] rounded-lg px-2.5 py-2">
                    <FileText size={11} className="text-muted mt-0.5 flex-shrink-0" />
                    <p className="text-xs text-muted leading-relaxed line-clamp-2">{shift.handoverNotes}</p>
                  </div>
                )}
              </div>
            </div>
            {shift.issues?.length > 0 && (
              <div className="mt-3 pl-16 space-y-1">
                ...
                {shift.issues.length > 2 && (
                  <p className="text-[10px] text-muted pl-4">+{shift.issues.length - 2} more</p>
                )}
              </div>
            )}
          </button>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={clsx('text-[10px] font-semibold px-2 py-0.5 rounded-full', styles[status] ?? styles['Not Started'])}>
      {status === 'Active' && (
